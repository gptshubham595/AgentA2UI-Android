...
Use the top tabs to switch between all three demos:

- **Playground**: type a prompt, receive A2UI JSON, and render a new native UI. **Live** mode debounces typing and regenerates automatically.
- **Flights JSON**: renders the provided flights JSON as native cards through generated A2UI messages.
- **Todo App**: renders an interactive todo app from A2UI messages with text input, checkbox state, buttons, and action callbacks.
...
The temporary Python OCI scripts are not packaged into Android. The Android app now has its own Ktor client that calls the same LiteLLM `/responses` endpoint when these Gradle properties are set:
...
The checked-in `gradle.properties` keeps the key empty and defaults to the local generator. For a real key, prefer `~/.gradle/gradle.properties` or environment variables with the same names.
